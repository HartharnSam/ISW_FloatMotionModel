...
Particle.r = LfLambda*wavelength;
...
for ii = 1:length(t)
    ...
    pcolor(x, z, ut);
    caxis(0.04*[-1 1]);
    cmocean('balance'); cb = colorbar; ylabel(cb, '$u (m/s)$','interpreter', 'latex')
    xlim([0 7])
    hold on
    ...
    legend('Float', 'Front Fluid', 'Rear Fluid','', 'Location', 'southeast');
    xticklabels([]);
